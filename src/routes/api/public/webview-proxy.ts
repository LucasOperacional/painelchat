...
function rewriteHtml(html: string, id: string, target: URL) {
 ...
 function ajusta(root){var sel="a[href],form[action],iframe[src],frame[src],object[data],embed[src]",els=[];if(root&&root.matches&&root.matches(sel))els.push(root);if(root&&root.querySelectorAll)els=els.concat(Array.prototype.slice.call(root.querySelectorAll(sel)));els.forEach(function(el){var at=el.tagName==="FORM"?"action":el.tagName==="OBJECT"?"data":el.tagName==="A"?"href":"src",v=el.getAttribute(at);if(v&&!/^(blob:|data:|javascript:|mailto:|tel:|#)/i.test(v)){var nv=w(v);if(nv!==v)el.setAttribute(at,nv);}});}
