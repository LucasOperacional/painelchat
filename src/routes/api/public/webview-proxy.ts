...
function rewriteHtml(html: string, id: string, target: URL) {
 ...
 var wo=window.open;window.open=function(u,n,o){if(typeof u==="string"&&u&&u.indexOf("blob:")!==0&&u.indexOf("data:")!==0)u=w(u);return wo.call(window,u,n,o);};
 function ajusta(root){var els=[];if(root&&root.matches&&root.matches("a[href],form[action]"))els.push(root);if(root&&root.querySelectorAll)els=els.concat(Array.prototype.slice.call(root.querySelectorAll("a[href],form[action]")));els.forEach(function(el){var at=el.tagName==="FORM"?"action":"href",v=el.getAttribute(at);if(v&&!/^(blob:|data:|javascript:|mailto:|tel:|#)/i.test(v))el.setAttribute(at,w(v));});}
 ajusta(document);new MutationObserver(function(ms){ms.forEach(function(m){if(m.type==="attributes")ajusta(m.target);else Array.prototype.forEach.call(m.addedNodes,ajusta);});}).observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:["href","action"]});
 document.addEventListener("click",function(e){var a=e.target&&e.target.closest&&e.target.closest("a[href]");if(!a)return;var h=a.getAttribute("href")||"";if(!/^(blob:|data:|javascript:|mailto:|tel:|#)/i.test(h))a.setAttribute("href",w(h));},true);
 document.addEventListener("submit",function(e){var form=e.target;if(form&&form.tagName==="FORM"){var a=form.getAttribute("action");if(a)form.setAttribute("action",w(a));}},true);
 var fs=HTMLFormElement.prototype.submit,fr=HTMLFormElement.prototype.requestSubmit;HTMLFormElement.prototype.submit=function(){var a=this.getAttribute("action");if(a)this.setAttribute("action",w(a));return fs.call(this);};if(fr)HTMLFormElement.prototype.requestSubmit=function(b){var a=this.getAttribute("action");if(a)this.setAttribute("action",w(a));return fr.call(this,b);};
