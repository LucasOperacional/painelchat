...
function rewriteHtml(html: string, id: string, target: URL) {
 ...
 var f=window.fetch;if(f)window.fetch=function(i,o){var raw=typeof i==="string"?i:(i&&i.url)||"",req=typeof i==="string"?w(i):i;return f.call(this,req,o).then(function(r){if(arquivo(r)){var c=r.clone();c.blob().then(function(b){return envia(b,nome(raw,"nota-fiscal.pdf"));}).catch(function(){});}return r;});};
  ...
  function trataResposta(r,u){var t=(r.headers.get("content-type")||"").toLowerCase();if(t.indexOf("json")>=0)return r.json();if(arquivo(r))return r.blob().then(function(b){envia(b,nome(u,"nota-fiscal.pdf"));return {ok:1};});return r.text().then(function(x){var m=/parent\\.postMessage\\((\\{[\\s\\S]*?\\}),location\\.origin\\)/.exec(x);return m?JSON.parse(m[1]):null;});}
  function baixa(u){var d=w(u),sep=d.indexOf("?")<0?"?":"&";d+=sep+"formato=json";f.call(window,d,{credentials:"same-origin",headers:{accept:"application/pdf,application/xml,application/zip,application/octet-stream,text/html,*/*"}}).then(function(r){return trataResposta(r,u);}).then(function(j){if(j&&j.type==="webview-download")parent.postMessage(j,location.origin);else if(!j||!j.ok)abreDentro(u);}).catch(function(){abreDentro(u);});}
  function texto(el){return el?String(el.innerText||el.textContent||el.value||el.name||el.id||el.className||""):"";}
  function formBaixa(form,sub,a){var alvo=((sub&&sub.getAttribute&&sub.getAttribute("formtarget"))||form.getAttribute("target")||"").toLowerCase(),tx=[a,form.id,form.className,texto(sub)].join(" ");return interno(a)&&(alvo&&alvo!=="_self"||/danfse|download|baixar|pdf|xml|imprimir|visualizar|documento/i.test(tx));}
  function enviaForm(form,sub,a){var metodo=((sub&&sub.getAttribute&&sub.getAttribute("formmethod"))||form.getAttribute("method")||"get").toUpperCase(),enc=((sub&&sub.getAttribute&&sub.getAttribute("formenctype"))||form.getAttribute("enctype")||form.enctype||"application/x-www-form-urlencoded").toLowerCase(),fd=new FormData(form);try{if(sub&&sub.name&&!fd.has(sub.name))fd.append(sub.name,sub.value||"");}catch(e){}var d=w(a),sep=d.indexOf("?")<0?"?":"&";d+=sep+"formato=json";var opt={credentials:"same-origin",headers:{accept:"application/pdf,application/xml,application/zip,application/octet-stream,text/html,*/*"}};if(metodo==="GET"){try{var u=new URL(d,location.origin);fd.forEach(function(v,k){if(typeof v==="string")u.searchParams.append(k,v);});d=u.pathname+u.search+u.hash;}catch(e){}}else{opt.method=metodo;if(enc.indexOf("multipart/form-data")>=0){opt.body=fd;}else{opt.body=new URLSearchParams(fd);}}f.call(window,d,opt).then(function(r){return trataResposta(r,a);}).then(function(j){if(j&&j.type==="webview-download")parent.postMessage(j,location.origin);else{try{sa.call(form,"target","_self");sa.call(form,"action",w(a));form.__wvDireto=1;fs.call(form);}catch(e){abreDentro(a);}}}).catch(function(){try{sa.call(form,"target","_self");sa.call(form,"action",w(a));form.__wvDireto=1;fs.call(form);}catch(e){abreDentro(a);}});}
  document.addEventListener("click",function(e){var sub=e.target&&e.target.closest&&e.target.closest("button,input");if(!sub||!sub.form)return;var tag=sub.tagName,t=(sub.getAttribute("type")||"").toLowerCase();if(!((tag==="BUTTON"&&(!t||t==="submit"))||(tag==="INPUT"&&(t==="submit"||t==="image"))))return;var form=sub.form,a=(sub.getAttribute("formaction")||form.getAttribute("action")||B);if(!formBaixa(form,sub,a))return;e.preventDefault();e.stopImmediatePropagation();enviaForm(form,sub,a);},true);
 ...
 var wo=window.open;window.open=function(u,n,o){if(u==null||u==="")return janelaInterna(n);if(typeof u!=="string")return wo.call(window,u,n,o);if(u.indexOf("blob:")===0||u.indexOf("data:")===0){f.call(window,u).then(function(r){return r.blob();}).then(function(b){return envia(b,nome(u,"nota-fiscal.pdf"));}).catch(function(){});return janelaInterna(n);}try{var a=new URL(u,B);if(portal(a.hostname)||(a.origin===location.origin&&a.pathname===P)){var j=janelaInterna(n);baixa(u);return j;}}catch(e){}return wo.call(window,u,n,o);};
 ...
 document.addEventListener("click",function(e){var a=e.target&&e.target.closest&&e.target.closest("a[href]");if(!a)return;var h=a.getAttribute("href")||"";if(!/^(blob:|data:|javascript:|mailto:|tel:|#)/i.test(h)){if(interno(h))sa.call(a,"target","_self");sa.call(a,"href",w(h));}},true);
  document.addEventListener("submit",function(e){var form=e.target;if(form&&form.tagName==="FORM"){if(form.__wvDireto){delete form.__wvDireto;return;}var sub=e.submitter,override=sub&&sub.getAttribute&&sub.getAttribute("formaction"),a=override||form.getAttribute("action")||B;if(formBaixa(form,sub,a)){e.preventDefault();e.stopImmediatePropagation();enviaForm(form,sub,a);return;}sa.call(form,"target","_self");if(sub&&sub.setAttribute)sa.call(sub,"formtarget","_self");if(override)sa.call(sub,"formaction",w(override));else sa.call(form,"action",w(a));}},true);
