...
function rewriteHtml(html: string, id: string, target: URL) {
  ...
  const shim = `<script>(function(){var P=${JSON.stringify(PROXY_PATH)},I=${JSON.stringify(id)},B=${JSON.stringify(target.toString())},H=${JSON.stringify(target.hostname)},seen=new WeakSet(),ultimoSubmit=null;
 ...
 var f=window.fetch;if(f)window.fetch=function(i,o){var raw=typeof i==="string"?i:(i&&i.url)||"",req=typeof i==="string"?w(i):i;return f.call(this,req,o).then(function(r){if(arquivo(r)){var c=r.clone();c.blob().then(function(b){return envia(b,nome(raw,"nota-fiscal.pdf"));}).catch(function(){});}return r;});};
  ...
  function texto(el){return el?String(el.innerText||el.textContent||el.value||el.name||el.id||el.className||el.getAttribute&&((el.getAttribute("title")||"")+" "+(el.getAttribute("aria-label")||""))||""):"";}
  function formBaixa(form,sub,a){var alvo=((sub&&sub.getAttribute&&sub.getAttribute("formtarget"))||form.getAttribute("target")||"").toLowerCase(),tx=[a,form.id,form.name,form.className,texto(form),texto(sub)].join(" ");return interno(a)&&(alvo&&alvo!=="_self"||/danfse|download|baixar|pdf|xml|imprimir|visualizar|documento|nota/i.test(tx));}
  ...
  document.addEventListener("click",function(e){var sub=e.target&&e.target.closest&&e.target.closest("button,input");if(!sub||!sub.form)return;var tag=sub.tagName,t=(sub.getAttribute("type")||"").toLowerCase();if((tag==="BUTTON"&&(!t||t==="submit"||t==="button"))||(tag==="INPUT"&&(t==="submit"||t==="image"||t==="button"))){ultimoSubmit=sub;}},true);
 ...
 protegeAlvo(HTMLAnchorElement.prototype,"href");protegeAlvo(HTMLFormElement.prototype,"action");
  ...
  document.addEventListener("click",function(e){var a=e.target&&e.target.closest&&e.target.closest("a[href]");if(!a)return;var h=a.getAttribute("href")||"";if(!/^(blob:|data:|javascript:|mailto:|tel:|#)/i.test(h)){if(interno(h))sa.call(a,"target","_self");sa.call(a,"href",w(h));}},true);
   document.addEventListener("submit",function(e){var form=e.target;if(form&&form.tagName==="FORM"){if(form.__wvDireto){delete form.__wvDireto;return;}var sub=e.submitter||((ultimoSubmit&&ultimoSubmit.form===form)?ultimoSubmit:null),override=sub&&sub.getAttribute&&sub.getAttribute("formaction"),a=override||form.getAttribute("action")||location.href||B;if(formBaixa(form,sub,a)){e.preventDefault();e.stopImmediatePropagation();enviaForm(form,sub,a);return;}sa.call(form,"target","_self");if(sub&&sub.setAttribute)sa.call(sub,"formtarget","_self");if(override)sa.call(sub,"formaction",w(override));else sa.call(form,"action",w(a));}},true);
   var fs=HTMLFormElement.prototype.submit,fr=HTMLFormElement.prototype.requestSubmit;HTMLFormElement.prototype.submit=function(){if(this.__wvDireto){delete this.__wvDireto;return fs.call(this);}var sub=(ultimoSubmit&&ultimoSubmit.form===this)?ultimoSubmit:null,a=(sub&&sub.getAttribute&&sub.getAttribute("formaction"))||this.getAttribute("action")||location.href||B;if(formBaixa(this,sub,a)){enviaForm(this,sub,a);return;}if(a){try{var u=new URL(a,B);if(portal(u.hostname))sa.call(this,"target","_self");}catch(_){}sa.call(this,"action",w(a));}return fs.call(this);};if(fr)HTMLFormElement.prototype.requestSubmit=function(b){if(this.__wvDireto){delete this.__wvDireto;return fr.call(this,b);}var sub=b||((ultimoSubmit&&ultimoSubmit.form===this)?ultimoSubmit:null),a=(sub&&sub.getAttribute&&sub.getAttribute("formaction"))||this.getAttribute("action")||location.href||B;if(formBaixa(this,sub,a)){enviaForm(this,sub,a);return;}if(a){try{var u=new URL(a,B);if(portal(u.hostname))sa.call(this,"target","_self");}catch(_){}sa.call(this,"action",w(a));}return fr.call(this,b);};
